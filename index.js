...
$('#comment-container').on('click', '.edit', function(event) {
    event.preventDefault();
    let parents = $(this).parents()[2];
    let child = $(parents).children()[2];
    $(child).toggleClass('displayNone')
});
$('#comment-container').on('click', '.submit2', function(event) {
    event.preventDefault();
...
$('#comment-container').on('click', '.delete-button', function(event) {
    event.preventDefault();
    let parents = $(this).parents()[2];
    $(parents).remove();
    console.log(parents)
